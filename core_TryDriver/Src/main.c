...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void set_bits_in_register(volatile uint32_t *reg, uint32_t code, uint8_t length, uint8_t offset)
{
    if (length > 32 ||
	offset > 31 ||
	offset + length > 32)
...
{
  /* USER CODE BEGIN 1 */
  const Motor_State right_states[] = {
      FORWARD,
      FORWARD,
      BACKWARD,
      BACKWARD,
...
#define STATES_LEN (sizeof(left_states) / sizeof(Motor_State))
  /* USER CODE END 1 */
  ...
  MX_TS_Init();
  /* USER CODE BEGIN 2 */
  set_bits_in_register(&GPIOC->MODER, 0b01, 2, IN1_PORT);
  set_bits_in_register(&GPIOC->MODER, 0b01, 2, IN2_PORT);
  ...
  while (1)
  {
    /* USER CODE END WHILE */
      ...
      driver_set_motor(&motor1, left_states[i]);
      driver_set_motor(&motor2, right_states[i]);
      
      wait_cycles(2 * SECOND);
