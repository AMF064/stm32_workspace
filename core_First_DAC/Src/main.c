...
int main(void)
{
  ...
  MX_DAC_Init();
  /* USER CODE BEGIN 2 */
  // Initialize DAC´s channel 2
  DAC->CR |= (1 << 16);
  GPIOA->MODER |= (1 << PORT_A5);
  GPIOA->MODER |= (1 << (PORT_A5 + 1));
  char states[] = {
		  0,
		  128,
		  255,
  ...
  uint8_t i = 0;
  // NOTE: this definition is correct in this case, since it is not a string literal
  // and the size of a char is 1
#define N_STATES (sizeof(states))
  /* USER CODE END 2 */
