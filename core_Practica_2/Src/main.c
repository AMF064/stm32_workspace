...
{
	__disable_irq();
	uint8_t value = ((GPIOA->IDR >> IR1) & 1);
	timer_set_pwm_dc(BUZZER_TIMER, BUZZER_CHANNEL, (value << 9));
	EXTI->PR |= 2;
	__enable_irq();
}
...
int main(void)
{
  ...
  MX_TIM4_Init();
  /* USER CODE BEGIN 2 */
  gpio_set_pin_mode(GPIOA, IR1, INPUT);
  gpio_set_pin_mode(GPIOB, BUZZER, AF);
  gpio_set_pin_alternate_function(GPIOB, BUZZER, AF2);

  // Habilitar EXTI1
  //SYSCFG->EXTICR[0] &= ~0xf;
  SYSCFG->EXTICR[0] &= ~(0xf << 4);
  //SYSCFG->EXTICR[0] &= ~((1 << 5) | (1 << 6)| (1 << 7)); SYSCFG->EXTICR[0] |= (1 << 4); //PA1 para EXTI1
  EXTI->IMR |= (1 << IR_INT);
  EXTI->FTSR |= (1 << IR_INT);
  EXTI->RTSR |= (1 << IR_INT);
  NVIC->ISER[0] |= (1 << IR_INT_ORDER);

  timer_configure_clock_signal(BUZZER_TIMER, 16e3, 1e3);
